...
void init(void)
{
	...
	g.nobjects=0;
  //--------------------------------------------------------------------
  //black sphere in the middle
  
  o = &g.object[g.nobjects];
  o->type = TYPE_SPHERE;
  vecMake(0.0, 200.0, 10.0, o->center);
  ...
  g.nobjects++;
	...
	g.nobjects++;
	//--------------------------------------------------------------------
	//red sphere sitting on top
	o = &g.object[g.nobjects];
	o->type = TYPE_SPHERE;
	vecMake(0.0, 235.0, 00.0, o->center);
	vecMake(.9,.0,.0, o->color);
	o->radius = 210.0;
	o->surface = SURF_NONE;
	g.nobjects++;
  //--------------------------------------------------------------------
	//white sphere sitting on bottom
	o = &g.object[g.nobjects];
	o->type = TYPE_SPHERE;
	vecMake(0.0, 185.0, 00.0, o->center);
  ...
  //height of cylinder
	...
	//
	//--------------------------------------------------------------------
	//cylinder sitting on floor outter
	orl = &g.object[g.nobjects];
	o->type = TYPE_CYLINDER;
	//o->inside = true;
	vecMake(0.0, 0.0, 0.0, o->center);
	vecMake(0.0, 0.0, -1.0, o->norm);
  ...
  //height of cylinder
	...
	//
	//--------------------------------------------------------------------
	//inner tall cylinder sitting on floor
	o = &g.object[g.nobjects];
	o->type = TYPE_CYLINDER;
	//o->inside = true;
	vecMake(0.0, 0.0, 0.0, o->center);
  ...
  //height of cylinder
	...
	//ring around the base of pokeball
	o = &g.object[g.nobjects];
	o->type = TYPE_RING;
	//o->inside = true;
	vecMake(0.0, 210.0, 100.0, o->center);
	vecMake(0.0, 25.0, -1.0, o->norm);
  ...
  */
	...
	vecMake(190.0, 250.0, 500.0, g.lightPos);
	vecMake(4.0, 300.0, 1100.0, g.from);
	vecMake(0.0, 110.0, 0.0, g.at);
